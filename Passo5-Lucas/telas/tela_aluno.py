...
class TelaAluno(TelaAbstrata):
    ...
    def pega_dados_notas_boletim(self):
        ...
        media_notas = {"matematica": float(semestre_mtm1 + semestre_mtm2) / 2,
                       "lingua_portuguesa": float(semestre_lp1 + semestre_lp2) / 2,
                       "geografia": float(semestre_geo1 + semestre_geo2) / 2,
                       "historia": float(semestre_hst1 + semestre_hst2) / 2, "ciencias": float(semestre_cie1 + semestre_cie2) / 2,
                       "artes": float(semestre_art1 + semestre_art2) / 2, "filosofia": float(semestre_fil1 + semestre_fil2) / 2,
                       "lingua_inglesa": float(semestre_ing1 + semestre_ing2) / 2,
                       "educacao_fisica": float(semestre_edf1 + semestre_edf2) / 2}
        self.close()
