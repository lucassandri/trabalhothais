...
from excecoes.entrada_invalida_exception import EntradaInvalidaException
...
class TelaFuncionario(TelaAbstrata):
    ...
    def validar_nome(self, nome):
        ...
        if any(x.isdigit() for x in nome):
            raise EntradaInvalidaException('Atenção! O nome não pode conter números ou caracteres especiais!')

    def validar_cargo(self, cargo):
        if cargo is None or cargo == '':
            raise EntradaInvalidaException('Atenção! Vocẽ não preencheu um campo!')
        if any(y.isdigit() for y in cargo):
            raise EntradaInvalidaException('Atenção! O cargo não pode conter números ou caracteres especiais!')

    def validar_salario(self, salario):
        if salario is None or salario == '':
            raise EntradaInvalidaException('Atenção! Vocẽ não preencheu um campo!')
        if any(z.isalpha() for z in salario):
            raise EntradaInvalidaException('Atenção! O salário não pode conter letras ou caracteres especiais!')

    def validar_email(self, email):
        if email is None or email == '':
            raise EntradaInvalidaException('Atenção! Vocẽ não preencheu um campo!')
        if not(any(w.alnum() for w in email)):
            raise EntradaInvalidaException('Atenção! O email não pode conter caracteres especiais!')
