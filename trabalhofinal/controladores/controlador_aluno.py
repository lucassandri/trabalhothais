...
class ControladorAluno(ControladorAbstrato):
    ...
    def _validar_dados_aluno(self, dados_aluno):
        ...
        if not dados_aluno["nome"].replace(" ", "").isalpha():
            raise CampoTextoException("Nome do aluno deve conter apenas letras")
        
        if not str(dados_aluno["matricula"]).isnumeric():
            raise CampoNumericoException("Matrícula deve conter apenas números")
        
        try:
            matricula = int(dados_aluno["matricula"])
            if matricula <= 0:
                raise CampoNumericoException("Matrícula deve ser um número inteiro positivo")
        except ValueError:
            raise CampoNumericoException("Matrícula inválida")
        ...
        if not dados_aluno["responsavel"].replace(" ", "").isalpha():
            raise CampoTextoException("Nome do responsável deve conter apenas letras")
        ...
        for aluno in self.__aluno_DAO.get_all():
            if str(aluno.matricula) == matricula_aluno:
                if aluno.boletim is not None:
                    ...
                    if novos_dados_notas_boletim is None:
                        return None
                    ...
                    if novos_dados_frequencia_boletim is None:
                        return None
    ...
    def incluir_aluno(self):
        ...
        except (CampoTextoException, CampoNumericoException, TypeError, ValueError) as e:
    ...
    def excluir_aluno(self):
        ...
        try:
            matricula_int = int(matricula_aluno)
            aluno = self.achar_aluno(matricula_int)
            
            if aluno is not None:
                self.__aluno_DAO.remove(matricula_int)
                self.__turma_DAO.remove(matricula_int)
            ...
            self.__tela_aluno.mostra_mensagem("Matrícula inválida. Use apenas números.")
        except Exception as e:
            self.__tela_aluno.mostra_mensagem(f"Erro ao excluir aluno: {str(e)}")
  
    def achar_aluno(self, matricula_aluno):
        for aluno in self.__aluno_DAO.get_all():
            if str(aluno.matricula) == str(matricula_aluno):
                return aluno
        return None
